const dashboardData = ({ dbs }) => {
  ...
  async function getMonthlyNumberOfSales() {
    ...
    WHERE B.date_transaction  = CURRENT_DATE;`;
      ...
      const quotationSales = parseInt(
        dailyRevenue.rows[0].quotation_daily_sales
      );
  ...
  async function getMonthlyRevenue() {
    try {
     ...
     WHERE B.date_transaction  >= DATE_TRUNC('month', CURRENT_DATE);`;
      ...
      const serviceSales = parseInt(
        monthlyRevenue.rows[0].service_monthly_sales
      );
      const quotationSales = parseInt(
  ...
  async function getTopSellingBrands() {
    try {
      const connect = await dbs();
      const sql = `SELECT C.brand as carBrand, SUM(C.price) as BrandSales, COUNT(C.car_id) FROM sales_invoice S INNER JOIN cars C on S.car_id = C.car_id GROUP BY C.brand ORDER by BrandSales DESC LIMIT 10`;
